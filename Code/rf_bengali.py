...
# Load and preprocess images with optional limit on forged samples per genuine
def load_images(root_dir, max_forg_per_gen=5):
    ...
    for unique_signature_folder in unique_signature_folders:
        ...
        if os.path.isdir(unique_folder_path):
            ...
            for gen in genuine_files:
                ...
                sampled_forged_files = random.sample(forged_files, min(max_forg_per_gen, len(forged_files)))
                for forg in sampled_forged_files:
...
# Main logic
with open("results_bengali_1.txt", "w") as f:
    with redirect_stdout(f):
        root_dir = "D:\\PyCharm 2024.3.5\\signature_verification\\Bengali"
        image_pairs, labels = load_images(root_dir, max_forg_per_gen=5)

        if not image_pairs:
            print("No image pairs found. Please check the directory structure.")
        else:
            features = []
            valid_labels = []
            ...
            for img1_path, img2_path in image_pairs:
                ...
                if f1 is not None and f2 is not None:
                    ...
                    valid_labels.append(labels[len(features)-1])
            ...
            print("After SMOTE:", dict(zip(*np.unique(labels, return_counts=True))))
            ...
            cv_scores = cross_val_score(best_rf, features, labels, cv=5)
...
plt.tight_layout()
plt.savefig("confusion_matrix_bengali.png")
plt.show()'''
import os
import numpy as np
from PIL import Image
...
with open("results_bengali.txt", "w") as f:
    with redirect_stdout(f):
        root_dir = "D:\\PyCharm 2024.3.5\\signature_verification\\Bengali"  # Adjust this path as needed
...
plt.title('Confusion Matrix')
plt.tight_layout()
plt.savefig("confusion_matrix_beng.png")  # Save the plot as an image
